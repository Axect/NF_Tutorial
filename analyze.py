...
def main():
    ...
    console.print(f"Mean: [{latent_samples[:, 0].mean():.4f}, {latent_samples[:, 1].mean():.4f}] (should be ~[0, 0])")
    console.print(f"Std: [{latent_samples[:, 0].std():.4f}, {latent_samples[:, 1].std():.4f}] (should be ~[1, 1])")
